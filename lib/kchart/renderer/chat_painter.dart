...
//    [yyyy, '-', mm, '-', dd, ' ', HH, ':', nn]
    ...
    int firstTime =
        datas.first?.date.isNotEmpty ? DateTime.parse(datas.first?.date).millisecondsSinceEpoch : 0;

    int secondTime =
        datas[1]?.date.isNotEmpty ? DateTime.parse(datas[1]?.date).millisecondsSinceEpoch : 0;
...
//    print("mStartIndex${mStartIndex},mStopIndex${mStopIndex}");
  ...
  void getVolMaxMinValue(KLineEntity item) {
    mVolMaxValue =
        max(mVolMaxValue, max(item.volume, max(item.MA5Volume, item.MA10Volume)));
    mVolMinValue =
        min(mVolMinValue, min(item.volume, min(item.MA5Volume, item.MA10Volume)));
  }
...
//    mSecondaryRect = Rect.fromLTRB(0, (mVolRect?.bottom??mMainRect.bottom )+ ChartStyle.childPadding, mWidth, (mVolRect?.bottom??mMainRect.bottom)  + secondaryHeight);
  ...
  void drawDate(Canvas canvas, Size size) {
    ...
    for (var i = 0; i < ChartStyle.gridColumns; ++i) {
      ...
      if (data != null) {
        TextPainter tp =
            getTextPainter(datas[index].date, ChartStyle.getDateTextStyle());
        y = size.height -
  ...
  void drawLongPressCrossLineText(
    ...
    TextPainter dateTp = getTextPainter(
        entity.date, TextStyle(color: Colors.white, fontSize: 10));
